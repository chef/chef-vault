...
do_install() {
  ...
  build_line "Setting GEM_PATH=$GEM_HOME"
  export GEM_PATH="$GEM_HOME"
  gem install --local chef-vault-*.gem --no-document --ignore-dependencies
  ...
  if ! grep -q 'APPBUNDLER_ALLOW_RVM' "${pkg_prefix}/bin/chef-vault"; then
    build_line "ERROR: binstub patch injection failed for ${pkg_prefix}/bin/chef-vault"
    return 1
  fi
